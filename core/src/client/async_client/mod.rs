...
impl ClientT for Client {
	async fn notification<'a>(&self, method: &'a str, params: Option<ParamsSer<'a>>) -> Result<(), Error> {
		...
		async {
			let raw = serde_json::to_string(&notif).map_err(Error::Serialization)?;
		...
		R: DeserializeOwned,
	{
		...
		async {
			let raw = serde_json::to_string(&RequestSer::new(&id, method, params)).map_err(Error::Serialization)?;
			tx_log_from_str(&raw, self.max_log_length);
			...
			serde_json::from_value(json_value).map_err(|_| ignore_serde_error_to_decode_error::<R>())
		}
		.instrument(trace.into_span())
		.await
	}
		...
		R: DeserializeOwned + Default + Clone,
	{
		...
		async {
			...
			let raw = serde_json::to_string(&batches).map_err(Error::Serialization)?;
			...
			let values: Result<_, _> = json_values
				.into_iter()
				.map(|val| serde_json::from_value(val).map_err(|_| ignore_serde_error_to_decode_error::<R>()))
				.collect();
			Ok(values?)
		}
		.instrument(trace.into_span())
...
impl SubscriptionClientT for Client {
		...
		N: DeserializeOwned,
	{
		...
		async {
			...
			let raw =
				serde_json::to_string(&RequestSer::new(&id, subscribe_method, params)).map_err(Error::Serialization)?;
